...
const folderStatus=document.createElement('p');folderStatus.id='folder-status';folderStatus.setAttribute('role','status');$('status').after(folderStatus);
async function checkFolder(){
 if(!session?.version)return;
 const card=session.cardId,version=session.version;
 try{const rows=await session.storage.request('c3_local_copies?card_id=eq.'+encodeURIComponent(card)+'&select=*');if(card!==session.cardId||version!==session.version)return;const r=rows[0];folderStatus.textContent=r&&r.version===version&&r.html_verified?'HTML saved to your folder · Version '+version+(r.json_verified?' · JSON history saved':' · JSON backup needs attention'):'Saved online · Waiting for the Mac to update your HTML file.';}catch{folderStatus.textContent='Saved online · Folder confirmation unavailable.';}
}
setInterval(checkFolder,15000);
...
async function openWorkspace(){
  ...
  const id=pageId || new URLSearchParams(location.search).get('card') || document.documentElement.dataset.cardId;
  if(pageId){
   if(!/^[a-f0-9-]{32,36}$/i.test(pageId))throw Error('Invalid page link');
   const rows=await session.storage.versions(pageId);
   if(rows.length)await session.open(pageId);
   else {const r=await fetch('./c3-template.html');if(!r.ok)throw Error('Template unavailable');session.newBlank(await r.text(),{});session.cardId=pageId;}
   $('mycards').hidden=true;$('new').hidden=true;
  }else if(id)await session.open(id);
  ...
  if(!id){$('mycards').hidden=true;$('new').hidden=true;$('save').disabled=true;say('Clean master · Create a new page in Use card stems db to fill in and save your own card.');}
}
...
$('save').onclick=async()=>{
 ...
 try{
  ...
  const saved=await session.save();updateHistory();checkFolder();
